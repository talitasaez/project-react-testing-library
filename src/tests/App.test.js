...
describe('Link do topo da aplicação', () => {
  ...
  // eslint-disable-next-line max-len
  test('Teste se a aplicação é redirecionada para a página About  ao clicar no About', () => {
    const { history } = renderWithRouter(<App />);
  ...
  // eslint-disable-next-line max-len
  test('Teste se a aplicação é redirecionada para a página Pokémons Favoritados ao clicar no Favorite Pokémons', () => {
    const { history } = renderWithRouter(<App />);
